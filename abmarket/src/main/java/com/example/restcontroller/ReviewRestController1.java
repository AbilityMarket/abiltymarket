...
public class ReviewRestController1 {
    ...
    public Map<String, Object> insertPost(
        ...
        try {
            ...
            // CHSTATE TDONE -> 후기 쓸 수 있게 설정
            ChatViewEntity chvEnt1 = chatViewRepository2.findByCrno(crno);
            // System.out.println(chvEnt.getChstate()); //N
            String chState = chvEnt1.getChstate();
            ChatViewEntity chvEnt2 = chatViewRepository2.findByChstateAndCrno(chState, crno);
            if(chvEnt2.getCrno() == crno && chvEnt2.getChstate().equals("TDONE")) {
                // 리뷰 유무 확인
                Long revno = chvEnt1.getReviewRevno();
                if(revno == null) {
                    ...
                    map.put("result", "이미 등록 된 후기가 있음");
                }
            } else {
                map.put("result", "거래 완료 되지 않은 채팅방");
            }
        } catch (Exception e) {
            e.printStackTrace();
            map.put("status", -1);
